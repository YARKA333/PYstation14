...
class Surface(pg.Surface):
    def __init__(self, *arg,**kwarg):
        ...
        if (isinstance(size, pg.Surface)):
            pg.Surface.__init__(self, size=size.get_size(), flags=size.get_flags())
            self.surface = self
            self.name='test'
            self.blit(size, (0, 0))

        else:
            pg.Surface.__init__(self, *arg, **kwarg)
            self.surface = self
            self.name = 'test'
    ...
    def __setstate__(self, state):
        ...
        pg.Surface.__init__(self, size=size, flags=flags)
